...
for i in range(0, nwin):
    ...
    print("Maximum likelihood at", round(beam_max[0],4), "\N{DEGREE SIGN},", round(beam_max[1],4), "\N{DEGREE SIGN},", round(beam_max[2],1), "km")
...
ax.add_patch(plt.Circle((x_max,y_max), facecolor = 'black', edgecolor ='white', radius=0.001))     # plot max likelihood position
ax.set_xlabel("Longitude")
ax.set_ylabel("Latitude")
ax.set_title("Likelihood location, map view")
for x,y in zip(net["lon"],net["lat"]): # plot stations
    triangle = RegularPolygon((x,y), facecolor = 'white', edgecolor ='black', numVertices=3, radius = 0.0015)
...
for xcorr_label, xcorr in ("Unshifted", PdF_beam.correlation_unshifted[i_win]), ("Shifted", PdF_beam.correlation_shifted[i_win]):
    ...
    # Cosmetics
    ax[0].set_title(xcorr_label+' cross-correlation envelopes', loc='left', va='bottom', size=10)
