...
def cross_correlation(
    ...
    for i in range(0, nslide):
        for j in range(0, nsta):
            ccZN[i, j] = signal.correlate(
                dataslideZ[i, j], dataslideN[i, j], mode="full", method="fft"
            )
            ccZE[i, j] = signal.correlate(
                dataslideZ[i, j], dataslideE[i, j], mode="full", method="fft"
            )
            ccNE[i, j] = signal.correlate(
                dataslideN[i, j], dataslideE[i, j], mode="full", method="fft"
            )
